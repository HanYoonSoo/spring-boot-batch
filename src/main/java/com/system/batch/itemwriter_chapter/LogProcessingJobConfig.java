package com.system.batch.itemwriter_chapter;
...
public class LogProcessingJobConfig {
    ...
    public SystemCommandTasklet mkdirTasklet(
        ...
        tasklet.setCommand("/bin/sh", "-c",
                String.format("mkdir -p %s %s && ls -al", collectedLogsPath, processedLogsPath));
    ...
    public SystemCommandTasklet scpTasklet(
        ...
        String sourceLogPath = "~/ecommerce_logs/" + date + ".log";

        StringJoiner commandBuilder = new StringJoiner(" && ");
        for (String host : List.of("localhost")) {
            String command;
            if ("localhost".equals(host)) {
                command = String.format("cp %s ./%s/%s.log",
                        sourceLogPath, processedLogsPath, host);
            } else {
                command = String.format("scp %s:%s ./%s/%s.log",
                        host, sourceLogPath, processedLogsPath, host);
            }
